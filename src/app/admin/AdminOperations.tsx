    ...
    return (
        <div className="mt-6 grid gap-6 xl:grid-cols-2">
            ...
            <section className="overflow-hidden rounded-3xl border border-[#d8ded8] bg-white">
                ...
                <div className="divide-y divide-[#e8ece8]">{[["registration_enabled", "Registration"], ["image_uploads_enabled", "Image uploads"], ["comments_enabled", "Comments"], ["public_profiles_enabled", "Public profiles"]].map(([key, label]) => <div key={key} className="flex items-center justify-between px-6 py-4"><span className="text-sm font-medium">{label}</span><button type="button" onClick={() => void togglePlatformSetting(key, !platformSettings[key])} className={`rounded-full px-3 py-1 text-xs font-semibold ${platformSettings[key] ? "bg-[#e9eee8] text-[#438268]" : "bg-[#f2ece8] text-[#a84f37]"}`}>{platformSettings[key] ? "ON" : "OFF"}</button></div>)}</div>
            </section>
            <section className="overflow-hidden rounded-3xl border border-[#d8ded8] bg-white">
