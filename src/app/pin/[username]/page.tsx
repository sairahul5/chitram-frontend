...
export default function PinPage() {
    ...
    if (!item) {
        return <main className="min-h-screen bg-[#f5f1e9]" aria-label="Loading post" />;
    }
            ...
            <article className="mx-auto mt-8 max-w-3xl overflow-hidden rounded-3xl border border-[#e4dcd3] bg-white shadow-sm">
                ...
                <div className="p-5 sm:p-7">
                    ...
                    {item.creatorUsername && (
                        <Link className="mt-5 inline-flex text-sm font-semibold text-[#d2643b] hover:underline" href={`/account/${item.creatorUsername}`}>
                            @{item.creatorUsername}
                        </Link>
                    )}
                </div>
